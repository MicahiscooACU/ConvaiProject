...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiNPCManager : MonoBehaviour
    {
        ...
        private void OnDrawGizmos()
        {
            ...
            float angleStep = visionConeAngle * 2f / arcResolution;

            Vector3 prevPoint = Quaternion.AngleAxis(-visionConeAngle, cameraTransform.up) * rayDirection * rayLength;
            for (int i = 1; i <= arcResolution; i++)
            {
                Vector3 nextPoint = Quaternion.AngleAxis(-visionConeAngle + angleStep * i, cameraTransform.up) * rayDirection * rayLength;
                Gizmos.DrawLine(rayOrigin + prevPoint, rayOrigin + nextPoint);
                prevPoint = nextPoint;
            }

            // Draw side rays of the cone
            Vector3 leftDir = Quaternion.AngleAxis(-visionConeAngle, cameraTransform.up) * rayDirection;
            Vector3 rightDir = Quaternion.AngleAxis(visionConeAngle, cameraTransform.up) * rayDirection;
            Gizmos.DrawLine(rayOrigin, rayOrigin + leftDir.normalized * rayLength);
            Gizmos.DrawLine(rayOrigin, rayOrigin + rightDir.normalized * rayLength);
        }
